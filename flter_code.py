...
def update_stock_pool(output_file="code.csv"):
    ...
    try:
        ...
        df_filtered.to_csv("full_code.csv",index=False)
        ...
        # 5. 格式化并保存
        series = df_filtered['代码'].astype(str).str.zfill(6)
        pd.DataFrame({'code': series}).to_csv(output_file, index=False)

        print(f"✅ 同步完成！")
        print(f"📊 符合条件（50亿-600亿市值  & 非ST）的股票共: {len(df_filtered)} 只")
